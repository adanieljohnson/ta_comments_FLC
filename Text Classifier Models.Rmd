...
Text classification problems can be broken down into three overlapping components:

1.  Feature selection and engineering
2.  Topic categorization and modeling
3.  Selecting an appropriate text classifier method

My approach for this text classification project is to use an initial codebook and corpus of hand-coded TA comments to test various approaches to semi- and fully automated classification methods and, if appropriate, revise the underlying classification scheme. 

This primer page describes the general mechanics of text classification and various classifier methods. Other pages outline the dataset structure (TA comments corpus), and initial codebook. 
...
*	 Using an empirically defined feature set to reverse engineer relevant features

The features below are used in natural language processing routinely. Not all can be used in all situations though.
...
Software for training and using word embeddings includes Tomas Mikolov's Word2vec, Stanford University's GloVe, fastText, Gensim, AllenNLP's Elmo, Indra and Deeplearning4j. 
...
##Categorization and Modeling Methods
**Curated Categories** (my term) are established by subject matter experts using an iterative process. The established categories can based on prior empirical studies, or constructed *de novo* through iterative coding.

**Topic Modeling** is a group of techniques to identify the groups of words (called a topic) from a collection of documents that contains best information in the collection. Topic modeling is more an iterative/generative process, and does not require any prior knowledge of the documents or relationships.
...
##Text Classifier Methods
There are numerous methods for classifying texts. A typical workflow for comparing classifiers would be to identify potentially usable classifier methods, then for each applicable classifier method:

*  Establish randomized training and test datasets
*  Train the classifier model, then evaluate that model on a test dataset

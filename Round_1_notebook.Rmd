...
### Data Structure
I have an Excel data table containing ~11,000 data rows, organized into these columns. The data have been de-duplicated already, so that the individual lines represent unique TA comments on student writing.

Name|Assigned|Description of What the Column Data Are
----------|----------|----------------------------------------------------------
unique.record|PI assigned|Unique line identifier for each data line in tables. SemesterYear.##### (example: Sp18.00012 is line 12 in dataset from Spring '18)
report.id|Qualtrics|R_XX value
sort|PI| (Can drop) Original sorting value. Duplicates of this number indicate a complex comment was split
report.title|Mixed|Title of the file from which a comment was extracted
student|WFU|(Still need to de-identify) Unique email address of student who wrote report
course|PI or TA|Course bins: 1113, 114, 214
ta|PI|(Still need to de-identify) Instructor marking paper
lab|PI|Topic of the lab on which report was written
tag|Student|First or second of semester. Observed that students often enter incorrectly. Revalidate 
type.TA|TA|Submission or revision version. Observed that students often enter incorrectly. Revalidate
grade.TA|TA|Assigned grade from the TA
grading.time|TA|Entered by TA, in minutes, or minutes.seconds
Rank|PI|(Can drop) Housekeeping data from coding. Used with Sort#
hypothesis.ok|TA|Yes or no for hypothesis present
data.ok|TA|Yes or no for data present
citation.ok|TA|Yes or no for citations in introduction and discussion
interpretation.ok|TA|Yes or no for data are interpreted
organization.ok|TA|Yes or no for properly organized into 8 sections
techflaws.ok|TA|Yes or no for whether report has technical errors
writing.ok|TA|Yes or no for whether report has writing errors
comments.incorporated|TA|Yes or no for whether student incorporated comments from submission into revision.
ta.comment|TA|Text of the actual extracted comment
code.subject|PI|below
code.structure|PI|below
code.locus|PI|below
code.scope|PI|below
code.tone|PI|below
code.notes|PI|below
...
These are the **Coded Characteristics.** Each has well-defined limited vocabulary of choices. Check codebook file for detailed descriptions.  

Name|Description|Status
------------|----------------------------------|-----------------------
code.subject|What is main subject?|Complete
code.structure|How is comment structured? Copy correction, etc.|Complete
...
## Support Data for De-Identification
I created a separate .xlsx file then exported to .csv that has the ACTUAL names of TAs and ACTUAL emails for students. There are two de-identifier columns: 

1.  There are 500 non-repeated unique IDs for TAs in format: **TA_7lL5KRzf**
2.  There are 10,000 non-repeated unique IDs for students in format: **Std_H6n5PxOy**
3.  There are additional columns for recording students, course IDs, and TAs for each semester from Spring'18 through Spring'20.

...
frequency_writing.subcolumns <- frequency_writing %>% select(1, 22:24)
```
...
base_data_tokenized_writing <- frequency_writing.subcolumns %>% 
...
base_data_tokenized_writing_sorted <- 
base_data_tokenized_writing %>% 
...
```{r}
word_ratios <- base_data_tokenized_writing %>%
...
```{r}
frequency3 <- base_data_tokenized_writing_sorted %>% 
...
```{r}
ggplot(frequency3, aes(`3. Technical and Scientific`, `4. Logic and Thinking`)) +
...
```{r}
word_ratios <- base_data_tokenized_writing_sorted %>%
  count(word, code.subject) %>%
  group_by(word) %>%
  filter(sum(n) > 0) %>%
  ungroup() %>%
...
base_data_tokenized_writing_sorted <- base_data_tokenized_writing %>% 
  group_by(code.subject) %>%
  count(code.subject, word, sort=TRUE) %>% 
  mutate(proportion = n / sum(n)) %>% 
  select(-n) %>% 
...
```{r}
cor.test(data = base_data_tokenized_writing_sorted[base_data_tokenized_writing_sorted$code.subject == "3. Technical and Scientific",], 
         ~ proportion + `2. Writing Quality`)
```

```{r}
cor.test(data = base_data_tokenized_writing_sorted[base_data_tokenized_writing_sorted$code.subject == "4. Logic and Thinking",], 
         ~ proportion + `2. Writing Quality`)
```
...
ggplot(base_data_tokenized_writing_sorted, aes(x = proportion, y = `2. Writing Quality`, color = abs(`2. Writing Quality` - proportion))) +

...
class(base_data_subset21[23,1]) #Should report as a character
```
...
```{r}
text_tibble <- tibble(line = seq_along(data), text = base_data_subset21_short)
```
...
The tibble must be a single-column table. See what happens when I try to create a tibble using a 3-column dataset.
```{r}
text_tibble_all <- tibble(line = seq_along(data), text = base_data_all_short)
```


NOW I KNOW that the tibble I created is going to have problems, because the data I read in does not include any information about WHICH row the words came from. How do I add the row values?

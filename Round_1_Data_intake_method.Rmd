...
sortedwords.comments.all.fraction <- sortedwords.comments.all %>% as_tibble() %>% mutate(
  fraction = n/total_words.all$total
)
```
...
sortedwords.comments.all <- base_data %>%
  unnest_tokens(word, ta.comment) %>%
  filter(!word %in% stop_words$word,
         !word %in% str_remove_all(stop_words$word, "'"),
         str_detect(word, "[a-z]")) %>%
...
Let's see this data graphically

```{r}
ggplot(sortedwords.comments.all.fraction, aes(fraction)) +
  geom_histogram(show.legend = FALSE, bins=300)

```


This block extracts the full sets of data text lines for unnested tokens of each of the coded subject groups. 
...
MODEL FOR HOW TO Remove the stopwords, count words in group, and calculate frequencies.
...
sortedwords.basic.fraction <- sortedwords.basic %>% as_tibble() %>% mutate(
  fraction = (n*100)/total_words.basic$total
)
```

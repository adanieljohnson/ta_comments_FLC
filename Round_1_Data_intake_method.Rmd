...
Create a single-column data frame of just the comments. Then check that the data was written to the file as characters not factors.
```{r}
base_data_column21 <- as.data.frame( base_data[,21], drop=FALSE, stringsAsFactors=FALSE) #assumes Col. 21 is the ta.commment column
class(base_data_column21[23,1]) #Should report as "character"
```

Also create a separate data frame for analysis. Contains **ta.comment** (Col. 21), **code.subject** (Col. 22) and **code.structure** (Col. 23). Again, check that written in data are characters, not factors. 
```{r}
base_data_3columns <- as.data.frame(base_data[,c(21:23)], drop=FALSE, stringsAsFactors=FALSE)
(base_data_3columns[33:35,1:3]) 
#First column should be row numbers, 2nd to 4th columns of class "chr"
```

Create a testing subset of 50-100 rows, and check first set...
```{r}
base_data_column21_short <-base_data_column21[c(1:50),]
base_data_3columns_short <-base_data_3columns[c(1:50),]
base_data_column21_short
```

...then check other set.
```{r}
base_data_3columns_short
```

***

#HERE IS THE former BREAKDOWN ZONE
...
```{r}
text_tibble <- tibble(line = seq_along(data), text = base_data_3columns_short)
```
